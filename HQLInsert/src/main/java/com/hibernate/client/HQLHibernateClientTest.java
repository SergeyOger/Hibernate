...
public class HQLHibernateClientTest {
	...
	public static void insertEmployee() {
		try (Session session = HibernateUtil.getSessionFactory().openSession()) {
			String HQL = "INSERT INTO Employee(name, doj, salary) SELECT name, doj, salary FROM BackupEmployee";
			Query<?> query = session.createQuery(HQL);
			session.beginTransaction();
			int exequteUpdate = query.executeUpdate();
			if (exequteUpdate > 0) {
				System.out.println(exequteUpdate + "records are inserted succesful");
			} else {
